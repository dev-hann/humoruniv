  ...
  group('MediaClassifier', () {
    group('classify', () {
      test('should classify .jpg as image', () {
        expect(MediaClassifier.classify('http://example.com/photo.jpg'),
            MediaType.image);
      });

      test('should classify .jpeg as image', () {
        expect(MediaClassifier.classify('http://example.com/photo.jpeg'),
            MediaType.image);
      });

      test('should classify .png as image', () {
        expect(MediaClassifier.classify('http://example.com/icon.png'),
            MediaType.image);
      });

      test('should classify .gif as image', () {
        expect(MediaClassifier.classify('http://example.com/anim.gif'),
            MediaType.image);
      });

      test('should classify .webp as image', () {
        expect(MediaClassifier.classify('http://example.com/photo.webp'),
            MediaType.image);
      });

      test('should classify .bmp as image', () {
        expect(MediaClassifier.classify('http://example.com/photo.bmp'),
            MediaType.image);
      });
      ...
      test('should be case insensitive for extensions', () {
        expect(MediaClassifier.classify('http://example.com/PHOTO.JPG'),
            MediaType.image);
        expect(MediaClassifier.classify('http://example.com/VIDEO.MP4'),
            MediaType.video);
      });
    });
    ...
    group('unwrapDownloadPhp', () {
      ...
      test('should return null for non-download URL', () {
        final result =
            MediaClassifier.unwrapDownloadPhp('http://example.com/photo.jpg');
    ...
    group('extractYoutubeId', () {
      ...
      test('should extract ID from short URL', () {
        expect(MediaClassifier.extractYoutubeId('https://youtu.be/dQw4w9WgXcQ'),
            'dQw4w9WgXcQ');
      });
    ...
    group('isMedia', () {
      ...
      test('should return true for video URLs', () {
        expect(
            MediaClassifier.isMedia('http://example.com/video.mp4'), isTrue);
      });
      ...
      test('should return true for audio URLs', () {
        expect(
            MediaClassifier.isMedia('http://example.com/audio.mp3'), isTrue);
      });

      test('should return false for regular links', () {
        expect(
            MediaClassifier.isMedia('https://example.com/page'), isFalse);
      });
